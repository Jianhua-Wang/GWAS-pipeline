...
def parseArguments():
   if len(sys.argv)<=1:
      sys.argv=\
      "missHetPlot.py $imiss $het $output $times_of_meanhet".split()
   parser=argparse.ArgumentParser()
   parser.add_argument('imiss', type=str, metavar='imiss'),
   parser.add_argument('het', type=str, metavar='het'),
   parser.add_argument('output', type=str, metavar='output'),
   parser.add_argument('times_of_meanhet', type=float, metavar='times_of_meanhet'),
...
het_min = het["meanHet"].mean()-args.times_of_meanhet*het["meanHet"].std()
het_max = het["meanHet"].mean()+args.times_of_meanhet*het["meanHet"].std()
plt_min = max(0,het_min-args.times_of_meanhet*het["meanHet"].std(),het["meanHet"].min())
plt_max = min(1,het_max+args.times_of_meanhet*het["meanHet"].std(),het["meanHet"].max())
y=het["meanHet"]
