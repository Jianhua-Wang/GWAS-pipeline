 ...
 *               2. Unphased panel (.vcf or .vcf.gz)
 *
 */
...
// Unphased VCF
unphased_vcf_ch = Channel.fromPath(params.input_vcf)
...
process plink_to_gen {
  ...
  file input_vcf from unphased_vcf_ch
  set file(bed), file(bim), file(fam) from plink_ch

  output:
  set val(chromosome), file("${plink_prefix}_chr${chromosome}.gen"), file("${plink_prefix}_chr${chromosome}.sample"), file("unphased_panel_chr${chromosome}.gz"), file("unphased_panel_chr${chromosome}.samples") into plink_gen_ch
  ...
  -vcf $input_vcf \
  -chr chr$chromosome \
  -gen unphased_panel_chr$chromosome
  """
}
...
imputeChromChunckChannel = plink_gen_ch.flatMap { chromosome, gensFile, sampleFile, unphasedGen, unphasedSample ->
   def results = []
   ...
   chunks.each { chunkStart, chunkEnd -> 
     results.push( [ chromosome, gensFile, sampleFile, unphasedGen, unphasedSample, chunkStart, chunkEnd] )
   }
...
process impute2 {
  ...
  input:
  set val(chromosome), file("${plink_prefix}_chr${chromosome}.gen"), file("${plink_prefix}_chr${chromosome}.sample"), file("unphased_panel_chr${chromosome}.gz"), file("unphased_panel_chr${chromosome}.samples"), val(chunkStart), val(chunkEnd) from imputeChromChunckChannel
  ...
  -g_ref unphased_panel_chr${chromosome}.gz \
...
process impute2Concat {
  ...
  """
  cat $imputedFiles > chr${chromosome}_Novo_unphased.imputed
  """
}