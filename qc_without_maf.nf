...
process removeQCPhase1 {
    ...
    script:
        ...
        plink --keep-allele-order --bfile temp1  --geno $cut_geno --make-bed --out temp3
        plink --keep-allele-order --bfile temp3  --hwe $cut_hwe --make-bed  --out $output 
        cat *log > logfile
        touch tmp.irem
        cat *.irem > ${output}.irem
        qc1logextract_without_maf.py logfile ${output}.irem $cut_mind $cut_geno $cut_maf $cut_hwe ${base}
        """
}
